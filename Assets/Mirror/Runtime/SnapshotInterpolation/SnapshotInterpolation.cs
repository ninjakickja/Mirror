...
namespace Mirror
{
    public static class SnapshotInterpolation
    {
            ...
            where T : Snapshot  
        {
            ...
            {
                if(buffer.Count > amount) //When debuging, used >= instead of >
                {
                    UnityEngine.Debug.Log($"Fail Older Check: {threshold} test, local time of snap: {buffer.Values[amount - 1].localTimestamp}");
                    ...
                    for(int i = amount; i < buffer.Count; i++)
                    {
                        ...
                        {
                            Debug.Log($"Enough buffer on 2nd check since we have snapshot {i}");
